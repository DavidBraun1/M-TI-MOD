...
private:
    ...
    void control_loop()
    {
        ...
        if (turning_mode_) {
            ...
            printf("Hindernis erkannt!\n");

            if (std::fabs(diff) > 0.05) { // Toleranz 0.05 rad ≈ 3°
                cmd.twist.angular.z = (diff > 0 ? 0.5 : -0.5);
            ...
            if (obstacle_detected_) {
                cmd.twist.linear.x = 0.0; // warten bis Scan_callback Ziel setzt
            } else {
